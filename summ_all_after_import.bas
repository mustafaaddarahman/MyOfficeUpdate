...
Sub CalculateSalaryPositions()
    ...
    With ws
        ...
        .Range("CS10").Formula = "=int(SUMIF(BF9:BF" & lastRow & ",""ãÓÊãÑ ÈÇáÎÏãÉ"", J9:J" & lastRow & "))"
        
        ' ÇáÑÇÊÈ ÇáÇÓãí ááãÌÇÒíä (5 ÓäæÇÊ)
        .Range("CT10").Formula = "=int(SUMIF(BF9:BF" & lastRow & ",""íÊãÊÚ ÈÇÌÇÒå ÎãÓ ÓäæÇÊ"", J9:J" & lastRow & "))"
        
        ' ÇáÑÇÊÈ ÇáÇÓãí áãÚíá ÇáãÊİÑÛ
        .Range("CU10").Formula = "=int(SUMIF(BF9:BF" & lastRow & ",""ÅÌÇÒÉ ãÚíá ÇáãÊİÑÛ"", J9:J" & lastRow & "))"
        
        ' ÇáÑÇÊÈ ÇáÇÓãí ááÌÇä ÇáØÈíÉ (Êã ÊÕÍíÍ ÇáÊÓãíÉ åäÇ)
        .Range("CV10").Formula = "=int(SUMIF(BF9:BF" & lastRow & ",""ÅÌÇÒÉ áÌÇä ØÈíå"", J9:J" & lastRow & "))"
        
        ' ÇáÑÇÊÈ ÇáÕÇİí (ÈÇÓÊÎÏÇã SUBTOTAL ááŞíã ÇáãİáÊÑÉ)
        .Range("CW10").Formula = "=int(SUBTOTAL(9, AW9:AW" & lastRow & "))"
        
        ' ÇáÑÇÊÈ ÇáÇÓãí ááÃãæãÉ (Ãæá 6 ÃÔåÑ)
        .Range("CX10").Formula = "=int(SUMIF(BF9:BF" & lastRow & ",""Çãæãå Çæá 6 ÇÔåÑ"", J9:J" & lastRow & "))"
        
        ' ÇáÅÌÇÒÇÊ ÈÏæä ÑÇÊÈ
        .Range("CY10").Formula = "=int(SUMIF(BF9:BF" & lastRow & ","" ÇÌÇÒÉ ÈÏæä ÑÇÊÈ"", J9:J" & lastRow & "))"
        
        ' ÇáÑÇÊÈ ÇáÇÓãí ááãäÓÈíä (ÊäÓíÈ)
        .Range("CZ10").Formula = "=int(SUMIF(BF9:BF" & lastRow & ",""ÊäÓíÈ"", J9:J" & lastRow & "))"
        ...
        .Range("DD10").Formula = "=int(SUMIF(BF9:BF" & lastRow & ",""Çãæãå ËÇäí 6 ÇÔåÑ"", J9:J" & lastRow & "))"
...
Sub UpdateLiveFormulas_Sheet1()
    ...
    With ws
        ...
        .Range("CC2").Formula = "=int(SUBTOTAL(9, X9:X" & lastRow & "))"
        .Range("CD2").Formula = "=int(SUBTOTAL(9, J9:J" & lastRow & "))"
        .Range("CE2").Formula = "=int(SUBTOTAL(9, AD9:AD" & lastRow & "))"
        .Range("CF2").Formula = "=int(SUBTOTAL(9, AE9:AE" & lastRow & "))"
        .Range("CG2").Formula = "=int(SUBTOTAL(9, AV9:AV" & lastRow & "))"
        .Range("CH2").Formula = "=int(SUBTOTAL(9, AW9:AW" & lastRow & "))"
        
        ' --- ÇáÓØÑ ÇáËÇáË (CJ3:CZ3) ---
        .Range("CJ3").Formula = "=int(SUBTOTAL(9, X9:X" & lastRow & "))"
        .Range("CK3").Formula = "=int(SUBTOTAL(9, AF9:AF" & lastRow & "))"
        .Range("CL3").Formula = "=int(SUM(AG9:AG" & lastRow & "))"
        .Range("CM3").Formula = "=int(SUBTOTAL(9, AH9:AH" & lastRow & "))"
        .Range("CN3").Formula = "=int(SUBTOTAL(9, AI9:AI" & lastRow & "))"
        .Range("CO3").Formula = "=int(SUBTOTAL(9, AJ9:AJ" & lastRow & "))"
        .Range("CP3").Formula = "=int(SUBTOTAL(9, AL9:AL" & lastRow & "))"
        .Range("CQ3").Formula = "=int(SUBTOTAL(9, AN9:AN" & lastRow & "))"
        .Range("CS3").Formula = "=int(SUBTOTAL(9, J9:J" & lastRow & "))"
        .Range("CT3").Formula = "=int(SUBTOTAL(9, N9:N" & lastRow & "))"
        .Range("CU3").Formula = "=int(SUBTOTAL(9, P9:P" & lastRow & "))"
        .Range("CV3").Formula = "=int(SUBTOTAL(9, R9:R" & lastRow & "))"
        .Range("CW3").Formula = "=int(SUBTOTAL(9, S9:S" & lastRow & "))"
        .Range("CX3").Formula = "=int(SUBTOTAL(9, T9:T" & lastRow & "))"
        .Range("CY3").Formula = "=int(SUBTOTAL(9, U9:U" & lastRow & "))"
        .Range("CZ3").Formula = "=int(SUBTOTAL(9, Y9:Y" & lastRow & "))"
        
        ' --- ÇáÓØÑ ÇáÑÇÈÚ (CC4:CH4) - ãÑÊÈØ ÈÇáÔÑØ CB4 ---
        .Range("CC4").Formula = "=int(SUMIF($BF$9:BF" & lastRow & ", $CB$4, X9:X" & lastRow & "))"
        .Range("CD4").Formula = "=int(SUMIF($BF$9:BF" & lastRow & ", $CB$4, J9:J" & lastRow & "))"
        .Range("CE4").Formula = "=int(SUMIF($BF$9:BF" & lastRow & ", $CB$4, AD9:AD" & lastRow & "))"
        .Range("CF4").Formula = "=int(SUMIF($BF$9:BF" & lastRow & ", $CB$4, AE9:AE" & lastRow & "))"
        .Range("CG4").Formula = "=int(SUMIF($BF$9:BF" & lastRow & ", $CB$4, AV9:AV" & lastRow & "))"
        .Range("CH4").Formula = "=int(SUMIF($BF$9:BF" & lastRow & ", $CB$4, AW9:AW" & lastRow & "))"
        
        ' --- ÇáÓØÑ ÇáÎÇãÓ (CJ5:CX5) ---
        .Range("CJ5").Formula = "=int(SUBTOTAL(9, AP9:AP" & lastRow & "))"
        .Range("CK5").Formula = "=int(SUBTOTAL(9, AR9:AR" & lastRow & "))"
        .Range("CL5").Formula = "=int(SUBTOTAL(9, AX9:AX" & lastRow & "))"
        .Range("CM5").Formula = "=int(SUBTOTAL(9, AT9:AT" & lastRow & "))"
        .Range("CN5").Formula = "=int(SUM(AU9:AU" & lastRow & "))"
        .Range("CO5").Formula = "=int(SUBTOTAL(9, BJ9:BJ" & lastRow & "))"
        .Range("CP5").Formula = "=int(SUBTOTAL(9, BO9:BO" & lastRow & "))"
        .Range("CQ5").Formula = "=int(SUBTOTAL(9, BQ9:BQ" & lastRow & "))"
        .Range("CS5").Formula = "=int(SUBTOTAL(9, AA9:AA" & lastRow & "))"
        .Range("CT5").Formula = "=int(SUBTOTAL(9, AC9:AC" & lastRow & "))"
        .Range("CU5").Formula = "=int(SUBTOTAL(9, BH9:BH" & lastRow & "))"
...
Sub UpdateSummaryFormula()
    ...
    ws.Range("CI4").Formula = "=int(SUMIF($BF$9:$BF$5000, $CB$4, AG9:AG5000))"
End Sub
