...
Private Sub CommandButton1_Click()
    ...
    Const HEADER_ROW As Long = 8 ' ’› «·⁄‰«ÊÌ‰
    Const DATA_START_ROW As Long = 9 ' »œ«Ì… «·»Ì«‰« 

    ' 1. «· Õﬁﬁ „‰ «·„œŒ·«  «·√”«”Ì…
    If TextBox1.Text = "" Then
        MsgBox "Ì—ÃÏ «Œ Ì«— «·√⁄„œ… «·„—«œ  —ÕÌ·Â«", vbExclamation
        Exit Sub
    End If
    ...
    cz = 0
    ' ÕÃ“ „”«Õ… ··„’›Ê›… «·„ƒﬁ … »‰›” ÕÃ„ «·»Ì«‰«  «·√’·Ì…
    ReDim tempArr(1 To UBound(dataArr, 1), 1 To UBound(colNums) + 1)

    ' 4. ⁄„·Ì… «·›· —… Ê«·»ÕÀ
    If filterValue = "" Then
        ' ≈–« ﬂ«‰ ’‰œÊﬁ «·»ÕÀ ›«—€«°  —ÕÌ· ﬂ· «·»Ì«‰«  «» œ«¡ „‰ «·’› 2 ›Ì «·„’›Ê›… («·–Ì ÂÊ ’› 9 ›Ì «·‘Ì )
    ...
    Else
        ...
        If filterCol = 0 Then
            MsgBox "·„ Ì „ «·⁄ÀÊ— ⁄·Ï ⁄„Êœ „ÿ«»ﬁ ·‹: " & ComboBox2.value, vbCritical
            Exit Sub
        End If
        ...
        For i = 2 To UBound(dataArr, 1)
            ...
            Else
                found = (InStr(1, Trim(CStr(cellValue)), filterValue, vbTextCompare) > 0)
            End If
    ...
    ' 5. «· Õﬁﬁ „‰ ÊÃÊœ ‰ «∆Ã
    If cz = 0 Then
        MsgBox "·«  ÊÃœ »Ì«‰«  „ÿ«»ﬁ… ··»ÕÀ: " & filterValue, vbExclamation
        Exit Sub
    End If

    ' 6. ‰ﬁ· «·‰ «∆Ã ≈·Ï „’›Ê›… ‰Â«∆Ì… »«·ÕÃ„ «·›⁄·Ì ··»Ì«‰«  «·„›· —…
    ...
    MsgBox " „ «· —ÕÌ· »‰Ã«Õ ·‹ " & cz & " ”Ã·« ", vbInformation
...
Public Sub ExportReportFiltered(arr As Variant, colNums() As Long, reportName As String, deptName As String)
    ...
    ' ===== Õ”«» «·„Ã«„Ì⁄ «·ﬂ·Ì… =====
    ws.Cells(lastDataRow + 1, 1).value = "«·„Ã«„Ì⁄ «·ﬂ·Ì…"
    ...
    For k = LBound(colNums) To UBound(colNums)
        If IsError(Application.Match(k + 1, excludeCols, 0)) Then
            ...
            For i = 1 To UBound(arr, 1)
                cellVal = arr(i, k + 1)
                If Not IsNumeric(cellVal) Or IsEmpty(cellVal) Then
                    isNumericCol = False
                    Exit For
                End If
            Next i
    ...
    '  ‰ŸÌ› «”„ «· ﬁ—Ì— „‰ √Ì «„ œ«œ ÌœÊÌ
    If InStr(reportName, ".xlsx") > 0 Then reportName = Replace(reportName, ".xlsx", "")
    
    '  ﬂÊÌ‰ «·«”„ «·ÃœÌœ: «”„ «· ﬁ—Ì— _ «·‘Â— _ «·”‰…
    fileNameWithDate = reportName & "_" & Format(Date, "mmmm_yyyy") & ".xlsx"
    savePath = saveFolder & fileNameWithDate
    
    ' «· Õﬁﬁ „‰ ÊÃÊœ „·› »‰›” «·«”„ ·⁄œ„ «·«” »œ«·
    fileNum = 1
    Do While Dir(savePath) <> ""
        savePath = saveFolder & Replace(fileNameWithDate, ".xlsx", "") & "_" & fileNum & ".xlsx"
        fileNum = fileNum + 1
    Loop
    ...
    If Err.Number <> 0 Then
        MsgBox "›‘· Õ›Ÿ «·„·›!  √ﬂœ „‰ ≈€·«ﬁ √Ì „·› „› ÊÕ »‰›” «·«”„ Ê„‰ ’·«ÕÌ«  «·„Ã·œ.", vbCritical
    Else
        MsgBox " „ «· —ÕÌ· Ê«·Õ›Ÿ »‰Ã«Õ:" & vbCrLf & savePath, vbInformation
    End If
    On Error GoTo 0
