VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} UserForm55 
   Caption         =   "UserForm55"
   ClientHeight    =   11868
   ClientLeft      =   120
   ClientTop       =   468
   ClientWidth     =   13212
   OleObjectBlob   =   "UserForm55.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "UserForm55"
...
Dim wsSrc As Worksheet
    ...
    For i = 2 To UBound(dataArr, 1)
        ...
        ' ≈–«  ÿ«»ﬁ «·›· —° «‰ﬁ· «·»Ì«‰«  Ê«Ã»—Â« · ﬂÊ‰ √—ﬁ«„« ’ÕÌÕ…
        If found Then
            cz = cz + 1
            For k = LBound(colNums) To UBound(colNums)
                CurrentVal = dataArr(i, colNums(k))
                
                ' ›Õ’ ≈–« ﬂ«‰  «·ﬁÌ„… —ﬁ„«° ‰ÕÊ·Â« ·—ﬁ„ ’ÕÌÕ ’—ÌÕ (Value)
                If IsNumeric(CurrentVal) And Not IsEmpty(CurrentVal) Then
                    tempArr(cz, k + 1) = Fix(CurrentVal)
                Else
                    tempArr(cz, k + 1) = CurrentVal
                End If
            Next k
        End If
    Next i
    ...
    If cz = 0 Then
        MsgBox "·«  ÊÃœ »Ì«‰«  „ÿ«»ﬁ… ··›· — «·„Œ «—", vbExclamation
        Exit Sub
    End If
    ...
    MsgBox " „  —ÕÌ· «·»Ì«‰«  »‰Ã«Õ (ﬂ√—ﬁ«„ ’ÕÌÕ… Ê»œÊ‰ ‰ﬁ’)", vbInformation, "⁄„«œ «·‰⁄Ì„Ì"
...
On Error Resume Next
    ...
    Dim filterColIndex As Long, cellValue As Variant
    ...
    For r = 2 To UBound(srcArr, 1)
        ...
        If filterColIndex = 0 Or filterValue = "" Then
            AddRow = True
        Else
            '  ÕÊÌ· «·ﬁÌ„… ·‰’ Ê«·»ÕÀ «·Ã“∆Ì »€÷ «·‰Ÿ— ⁄‰ ‰Ê⁄ «·»Ì«‰« 
            cellValue = CStr(srcArr(r, filterColIndex))
            If InStr(1, cellValue, filterValue, vbTextCompare) > 0 Then
                AddRow = True
            End If
        End If
