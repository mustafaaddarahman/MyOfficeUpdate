Attribute VB_Name = "raf3_yead"
Sub raf3_yead_1_22()
UserForm1.TextBox101.value = 0
'=======================«·—« » «·’«›Ì

UserForm1.TextBox49.value = 0
UserForm1.TextBox14.value = val(UserForm1.ComboBox6) * val(UserForm1.TextBox10)
UserForm1.TextBox16.value = val(UserForm1.ComboBox7) * val(UserForm1.TextBox10)
UserForm1.TextBox18.value = val(UserForm1.ComboBox8) * val(UserForm1.TextBox10)
UserForm1.TextBox20.value = val(UserForm1.TextBox22) * 10000
UserForm1.TextBox27.value = val(UserForm1.ComboBox10) * val(UserForm1.TextBox10)
UserForm1.TextBox101.value = 0
UserForm1.TextBox29.value = val(UserForm1.ComboBox11) * val(UserForm1.TextBox10)
UserForm1.TextBox33.value = 0
'==========================„Ã„Ê⁄ «·„Œ’’« 
Dim numlong As Long
Dim numlong1 As Long
numlong = 10000
UserForm1.TextBox20.value = numlong1
If UserForm1.TextBox22.value >= 5 Then
numlong1 = 40000
UserForm1.TextBox20.value = numlong1
ElseIf UserForm1.TextBox22.value <= 4 Then
UserForm1.TextBox20.value = val(UserForm1.TextBox22) * numlong
End If
'==================================== «–« ﬂ«‰ „Œ’’«  „Â‰ÌÂ „ÊÃÊœÂ „„ﬂ‰ «‰  Ã„⁄ „⁄ „Œ’’«  «·”Ê«ﬁ

UserForm1.TextBox30.value = val(UserForm1.TextBox14) + val(UserForm1.TextBox105) + val(UserForm1.TextBox16) + val(UserForm1.TextBox18) + val(UserForm1.TextBox19) + val(UserForm1.TextBox21) + val(UserForm1.TextBox20) _
+ val(UserForm1.TextBox25) + val(UserForm1.TextBox27) + val(UserForm1.TextBox29) + val(UserForm1.TextBox60) + val(UserForm1.TextBox84) + val(UserForm1.TextBox86 + val(UserForm1.TextBox111))





'============================„Ã„Ê⁄ «·«” Õﬁ«ﬁ« 


UserForm1.TextBox31.value = val(UserForm1.TextBox10) + val(UserForm1.TextBox30)






'============================«· Êﬁ›«  «· ﬁ«⁄œÌÂ
UserForm1.TextBox32.value = val(UserForm1.TextBox10) * 0.1
UserForm1.TextBox33.value = 0


'==================== „»·€ «·€Ì«»
Call state_tax


'===============
 '  «·«” ﬁÿ«⁄« 
If UserForm1.TextBox8.value <= 6 Then

 UserForm1.TextBox42.value = 1500
 ElseIf UserForm1.TextBox8.value >= 7 Then
 UserForm1.TextBox42.value = 1000
End If
UserForm1.TextBox46.value = Int(val(UserForm1.TextBox10) * 0.0025)

Dim sumastktaa As Double
sumastktaa = Int(val(UserForm1.TextBox24) + val(UserForm1.TextBox32) + val(UserForm1.TextBox33) + val(UserForm1.TextBox34) + val(UserForm1.TextBox35) + val(UserForm1.TextBox36) _
+ val(UserForm1.TextBox38) + val(UserForm1.TextBox40) + val(UserForm1.TextBox42) + val(UserForm1.TextBox44) + val(UserForm1.TextBox46) _
 + val(UserForm1.TextBox50) + val(UserForm1.TextBox62) + val(UserForm1.TextBox83) + val(UserForm1.TextBox85) + val(UserForm1.TextBox111))
 
 
 UserForm1.TextBox48.value = Int(sumastktaa)
UserForm1.TextBox49.value = Int(val(UserForm1.TextBox31) - val(UserForm1.TextBox48))
UserForm1.TextBox48.value = Int(val(UserForm1.TextBox47) + sumastktaa)
'=======================«·—« » «·’«›Ì
UserForm1.TextBox49.value = 0


UserForm1.TextBox46.value = Int(val(UserForm1.TextBox10) * 0.0025)




'=======================================================
Call update_data_acount_22
End Sub
